      ...
      if self.outliers_count != 0:
          ...
          if self.date_column != None:
              if self.rolling_period == 1:
               ...
               for index, row in self.df_outliers.iterrows(): 
                    st.write(f"Observation {index}, with a value of {row[str(self.target_column)+'_VALUE']} in the target column {self.target_column}. The sensitivity was {self.sensitivity} and the threshold range ({self.lower_threshold} - {self.upper_threshold}) has been crossed.")
