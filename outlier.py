      ...
      if self.outliers_count != 0:
          ...
          if self.rolling_period == 1:
              ...
              for row in self.df_outliers:
                  st.write(f'In date {row[self.date_column]} the target colmn value {self.target_column} is {self.target_column} when the threshold with sensitivity of {self.sensitivity} was ({self.upper_threshold}-{self.lower_threshold})')
